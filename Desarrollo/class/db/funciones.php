...
function obtenerTiposRamo($tipoUsuario)
{
  ...
  if($tipoUsuario == 2 || $tipoUsuario == 3)
  {
    $sqlf = "SELECT rt.id,rt.tipo 
             ...
             WHERE rt.soloDepto = false;";
  }
  elseif($tipoUsuario == 4)
  {
    $sqlf = "SELECT rt.id,rt.tipo 
              FROM Ramo_Tipo AS rt
             WHERE rt.soloDepto = true;";
  }
  $resf = $mysqlif->prepare($sqlf);
  $resf->execute();
...
function verRamosImpartidos($codigoCarrera,$codigoSemestre) {
    global $mysqli,$db_host,$db_user,$db_pass,$db_database;
    $mysqli = @new mysqli($db_host, $db_user, $db_pass, $db_database);
    $sql = "SELECT c.Regimen,r.Codigo,r.Nombre,r.Tipo,rt.Abreviacion,ctr.Semestre,c.Periodo,rt.soloDepto
    ...
    $res = $mysqli->prepare($sql);
    $res->execute();
    $res->bind_result($regimen,$codigoRamo,$nombreRamo,$tipo,$tipoAbreviacion,$semestreRamo,$periodo,$soloDepto);
    $flag = 0;
    if($periodo == 1)
    ...
    while($res->fetch())
    {
      ...
      $mysqli3 = @new mysqli($db_host, $db_user, $db_pass, $db_database);
      $sql3 = "CALL seccionesCreadasOtroNumero('{$codigoRamo}','{$codigoCarrera}','{$codigoSemestre}','{$regimen}')";
      $res3 = $mysqli3->prepare($sql3);
      $res3->execute();
      ...
      {
        echo '<tr><td class="mid">'.$semestreRamo.'</td><td>'.$codigoRamo.'</td><td>'.$nombreRamo.'</td><td class="mid"></td><td class="mid"></td><td class="mid"></td><td class="mid"></td></tr>';
      }
    }
    if($flag == 0)
...
function verSeccionesCreadasOtros($codigoRamo,$codigoSemestre,$codigoCarreraMia,$regimen) {
  global $mysqli,$db_host,$db_user,$db_pass,$db_database;
  $mysqli = @new mysqli($db_host, $db_user, $db_pass, $db_database);
  $sql = "SELECT s.Id,s.Numero_Seccion,s.NRC,s.Codigo_Ramo,r.Nombre,s.Codigo_Carrera,s.Codigo_Semestre,s.Vacantes
           FROM Seccion AS s
...
function verHorario($codigoCarrera,$codigoSemestre,$numeroSemestre)
{
  ...
  if($regimen == 'D')
    ...
    $sql2 = "SELECT m.Modulo,m.Inicio,m.Termino
           FROM Modulo AS m 
          WHERE m.Regimen = '{$regimen}' ORDER BY m.Modulo ASC;"; 
  $res2 = $mysqli2->prepare($sql2);
  $res2->execute();
...
function verRamosDepartamento() {
    ...
    $mysqliVRD = @new mysqli($db_host, $db_user, $db_pass, $db_database);
    $sqlVRD = "SELECT r.Codigo,r.Nombre,r.Periodo,r.Teoria,rt.Abreviacion,r.Ayudantia,r.Laboratorio,r.Taller,r.Creditos
             FROM Ramo AS r 
             INNER JOIN Ramo_Tipo AS rt ON rt.Id = r.Tipo
            WHERE rt.soloDepto = 1 ORDER by r.Codigo;";
    $resVRD = $mysqliVRD->prepare($sqlVRD);
    $resVRD->execute();
    $resVRD->bind_result($codigo,$nombre,$periodo,$teoria,$tipo,$ayudantia,$laboratorio,$taller,$creditos);  
    while($resVRD->fetch())
    {
      if($periodo == 1 && $codigoSemestre != 0)
      {
	    if(($resp = comprobarRamoDepto($codigo,$codigoSemestre,'D')) == false)
          $formD = '<form method="post" name="crearSeccion" target="_self"><input type="hidden" name="hiddenRegimen" value="D"></input><input type="hidden" name="hiddenCodigoRamo" value="'.$codigo.'"></input><input type="hidden" name="hiddenCodigoSemestre" value="'.$codigoSemestre.'"></input><input type="submit" name="submit" value="Dictar"></input></form>';
		else
		  $formD = '<input type="submit" name="submit" value="Dictado" disabled></input>';
		if(($resp = comprobarRamoDepto($codigo,$codigoSemestre,'V')) == false)
		  $formV = '<form method="post" name="crearSeccion" target="_self"><input type="hidden" name="hiddenRegimen" value="V"></input><input type="hidden" name="hiddenCodigoRamo" value="'.$codigo.'"></input><input type="hidden" name="hiddenCodigoSemestre" value="'.$codigoSemestre.'"></input><input type="submit" name="submit" value="Dictar"></input></form>';
		else
		  $formV = '<input type="submit" name="submit" value="Dictado" disabled></input>';
      }
      else if($periodo == 1 && $codigoSemestre == 0)
      {
        $formD = 'Semestre cerrado.';
		$formV = 'Semestre cerrado.';
      }
      else if($periodo == 2 && $codigoTrimestre != 0)
      {
	    if(($resp = comprobarRamoDepto($codigo,$codigoTrimestre,'D')) == false)
          $formD = '<form method="post" name="crearSeccion" target="_self"><input type="hidden" name="hiddenRegimen" value="D"></input><input type="hidden" name="hiddenCodigoRamo" value="'.$codigo.'"></input><input type="hidden" name="hiddenCodigoSemestre" value="'.$codigoTrimestre.'"></input><input type="submit" name="submit" value="Dictar"></input></form>';
		else
		  $formD = '<input type="submit" name="submit" value="Dictado" disabled></input>';
		if(($resp = comprobarRamoDepto($codigo,$codigoTrimestre,'V')) == false)
		  $formV = '<form method="post" name="crearSeccion" target="_self"><input type="hidden" name="hiddenRegimen" value="V"></input><input type="hidden" name="hiddenCodigoRamo" value="'.$codigo.'"></input><input type="hidden" name="hiddenCodigoSemestre" value="'.$codigoTrimestre.'"></input><input type="submit" name="submit" value="Dictar"></input></form>';
		else
		  $formV = '<input type="submit" name="submit" value="Dictado" disabled></input>';
      }
      else if($periodo == 2 && $codigoTrimestre == 0)
      {
        $formD = 'Trimestre cerrado.';
		$formV = 'Trimestre cerrado.';
      }
	  else
	  {
	    $formD = 'Error.';
      ...
      echo '<tr><td>'.$codigo.'</td><td>'.$nombre.'</td><td class="mid">'.$formD.'</td><td class="mid">'.$formV.'</td></tr>';
    }
    if(!isset($codigo))
      echo '<tr><td>No hay ramos.</td></tr>';
    $resVRD->free_result();
  ...
  function comprobarRamoDepto($codigoRamo,$codigoSemestre,$regimen)
  {
    global $mysqli,$db_host,$db_user,$db_pass,$db_database;
    $mysqliCRD = @new mysqli($db_host, $db_user, $db_pass, $db_database);
    $sqlCRD = "SELECT s.Id
             FROM Seccion AS s
            WHERE s.Codigo_Ramo = '{$codigoRamo}' AND s.Codigo_Carrera = 'DEPTO' AND s.Codigo_Semestre = '{$codigoSemestre}' AND s.Regimen = '{$regimen}';";
    $resCRD = $mysqliCRD->prepare($sqlCRD);
    $resCRD->execute();
    $resCRD->bind_result($id);
    if($resCRD->fetch())
    {
      $resCRD->free_result();
      return true;
...
function profesorInfo($rutProfesor)
{
  global $mysqli,$db_host,$db_user,$db_pass,$db_database;
  $mysqlio = @new mysqli($db_host, $db_user, $db_pass, $db_database);
  $sqlo = "SELECT p.Nombre,p.Profesor_Grado
            FROM Profesor AS p
           WHERE p.Rut_Profesor = '{$rutProfesor}';";
  $reso = $mysqlio->prepare($sqlo);
  $reso->execute();
  $reso->bind_result($nombreProfesor,$profesorGrado);
  if($reso->fetch())
  {
    $profesor[0]["nombreProfesor"] = $nombreProfesor;
	$profesor[0]["profesorGrado"] = $profesorGrado;
  }
  $reso->free_result();
  return $profesor;
}

function verClasesSinHorarioDepto($regimen)
{
  $codigoSemestre = obtenerSemestreDepartamento();
  $codigoTrimestre = obtenerTrimestreDepartamento(); 
  $codigoCarrera = 'DEPTO';	
  ...
  $sql = "SELECT c.id,s.Codigo_Ramo,s.Numero_Seccion,c.Clase_Tipo,c.RUT_Profesor
           FROM Clase AS c
           INNER JOIN Seccion AS s ON s.Codigo_Carrera = '{$codigoCarrera}' AND (s.Codigo_Semestre = '{$codigoSemestre}' OR s.Codigo_Semestre = '{$codigoTrimestre}') AND s.Regimen = '{$regimen}'
          WHERE c.Seccion_Id = s.Id AND (s.Codigo_Semestre = '{$codigoSemestre}' OR s.Codigo_Semestre = '{$codigoTrimestre}') AND c.Dia IS NULL AND c.Modulo_Inicio IS NULL AND c.Modulo_Termino IS NULL ORDER BY s.Codigo_Ramo,s.Numero_Seccion;"; 
  $res = $mysqli->prepare($sql);
  $res->execute();
  $res->bind_result($idClase,$codigoRamo,$numeroSeccion,$tipoClase,$profesor);
  $lastSeccion = 0;
...
function verHorarioDepto($regimen)
{
  $codigoCarrera = 'DEPTO';
  $codigoSemestre = obtenerSemestreDepartamento();
  $codigoTrimestre = obtenerTrimestreDepartamento(); 
      ...
      if($regimen == 'D') 
      {
        ...
        for($i = 0;$i<5;$i++)
        {
          ...
          $sql = "SELECT c.Id,s.Codigo_Ramo,s.Numero_Seccion,c.Clase_Tipo,c.Dia,c.Modulo_Inicio,'SI',c.RUT_Profesor
                   FROM Seccion AS s
                   INNER JOIN Clase AS c ON c.Seccion_Id = s.Id AND c.Modulo_Inicio = '{$moduloAnterior}' AND c.Modulo_Termino = '{$modulo}' AND c.Dia = '{$dias[$i]}'
                  WHERE s.Codigo_Carrera = 'DEPTO' AND (s.Codigo_Semestre = '{$codigoSemestre}' OR s.Codigo_Semestre = '{$codigoTrimestre}') AND s.Regimen = '{$regimen}';"; 
          $res = $mysqli->prepare($sql);
          $res->execute();
          $res->bind_result($claseId,$codigoRamo,$numeroSeccion,$claseTipo,$dia,$moduloInicio,$moduloTermino,$profesor);
          $flag = 0;
